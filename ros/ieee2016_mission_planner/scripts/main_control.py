    ...
    def begin(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.current_state == 1:
                ...
                camera = self.cam_list[self.map_version - 2]
                camera.activate()
                train = TrainBoxes(camera, self.w)
                train_box_colors = train.get_box_order(["red", "blue", "yellow", "green"], ["box_1", "box_2", "box_3", "box_4"], 16)
                camera.deactivate()
                print "Train Box Locations: %s" % (train_box_colors)
                ...
                The goals of this state are:
                    ...
                    2. Get the colors that the grippers on ee1 will pick up.camera
