...
class Image():
	...
	def reduce_colors(self, image_color_depth):
		...
		if (self.hold_redux_frame):
			...
			working_frame = cv2.medianBlur(working_frame, 3)

			# Scales the image down to improve K-Means performance
			...
			ret, labels, centers = cv2.kmeans(working_frame, image_color_depth, criteria, 10, cv2.KMEANS_RANDOM_CENTERS)

			# Pulls a new frame to apply the centers to
			self.resize()
			working_frame = cv2.bilateralFilter(self.frame, 9, 75, 75)
			working_frame = cv2.medianBlur(working_frame, 3)
			...
			centers = centers[np.lexsort(np.transpose(centers)[::-1])]


			matching = np.digitize(working_frame[:, 0], centers[:, 0], right = False)
			working_frame = centers[np.subtract(matching.flatten(), [1])]

# 			matching = []
# 			for pixel in range(len(working_frame)):
# 				distances = centers - working_frame[pixel]
# 				matching.append(np.argmin((distances * distances).sum(1)))
# 			working_frame = centers[matching]
