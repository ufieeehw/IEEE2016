__kernel void trace( __global const float *p,  
    ...
    float3 ray_origin = {p[i*3], p[i*3+1],0};
    ...
    for(int a = 0; a < angle_cnt; a++){
        ...
        float3 ray_direction = {cos(theta), sin(theta),0};

        for(int w = 0; w < wall_cnt; w++){
            float3 point1 = {walls[4*w],walls[4*w+1],0};
            float3 point2 = {walls[4*w+2],walls[4*w+3],0};

            v1 = ray_origin - point1;
            v2 = point2 - point1;
            float3 v3 = {-ray_direction.s1, ray_direction.s0, 0};
    ...
    ret_weights[i] = error/errors_measured;
}