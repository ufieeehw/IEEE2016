...
class GPUAccMap():
    def __init__(self):
        ...
        # Load .cl program
        filename = os.path.join(os.path.dirname(__file__), 'particle_filter.cl')
        f = open(filename, 'r')
        fstr = "".join(f.readlines())
        cl.Program(self.ctx, fstr).build()
...
class GPUAccFilter(Thread):
    def __init__(self, p_count, center, radius, heading_range, mW):
